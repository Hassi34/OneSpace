from .eda import EDA , Evaluation
from .common import get_unique_filename
from .common_eda import get_num_cols
import os
class Plots(object):
    def __init__(self, data, target, parent_dir, plots_dir):
        self.eda = EDA(data)
        self.eda_dir = os.path.join(parent_dir, plots_dir,"EDA", get_unique_filename("EDA"))
...
class Eval:
    def __init__(self, y_val, target_names , parent_dir, plots_dir):
        ...
        #self.vizpool_eval = Evaluation(self.y_val, y_predicted)
        self.y_val = y_val
        self.eval_dir = os.path.join(parent_dir, plots_dir, "Evaluation", get_unique_filename("Evaluation"))
    ...
    def confusion_matrix(self, y_predicted, model):
        ...
        plt_path = os.path.join(self.eval_dir, f"{self.model}_confusion_matrix_pct")
        try:
            plt.savefig(plt_path)
        except AttributeError:
            pass
        plt = self.vizpool_eval.confusion_matrix(y_predicted, self.target_names, normalize=False,
             title = f"Confusion Matrix for {self.model}")
        plt_path = os.path.join(self.eval_dir, f"{self.model}_confusion_matrix_count")
        try:
            plt.savefig(plt_path)
        except AttributeError:
            pass
    def feature_importance(self, model, X_val):
        plt = self.vizpool_eval.feature_importance(model, X_val, self.y_val)
        plt_path = os.path.join(self.eval_dir, f"{self.model}_feature_importance")
        try:
            plt.savefig(plt_path)
        except AttributeError:
            pass
