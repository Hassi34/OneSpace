...
import pandas as pd
import numpy as np
from .common_eda import get_num_cols, get_cat_cols
import itertools
...
class EDA:
    ...
    def pie_bar(self, hue, kind="barh", color='red', fig_width = 14, fig_height = 8):
        plt.figure(figsize=(fig_width,fig_height))
        fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, sharey=False, figsize=(fig_width,fig_height))
        labels_list = []
        explode_list = []
    ...
    def histogram(self, col_to_plot = "default", hue = None, title = "Distribution Plot",fig_width = 25, fig_height = 15, kde=True):
        ...
        for i in range((len(num_cols))):
            ...
            sns.histplot(data = self.df, x = num_cols[i], hue = hue, element="step", kde=kde)
        plt.title(title)
        plt.tight_layout()
        return plt

    def boxplot(self, col_to_plot = "default", hue = None, fig_width = 20, fig_height = 10):
        sns.set(font_scale=1)
        num_cols, columns = get_num_cols(self.df, col_to_plot)
        plt.figure(figsize=(fig_width,fig_height))
        for i in range((len(num_cols))):
    ...
    def violinplot(self,hue, col_to_plot = "default",inner="quart", fig_width = 15,
        ...
        data = pd.melt(data,id_vars= y.name,
                            ...
                            value_name='value')
        plt.figure(figsize=(fig_width,fig_height))
        sns.violinplot(x="features", y="value", hue= y.name, data=data,split=False, inner=inner)
        plt.title(title)
    ...
    def joint_plot(self, var1 , var2, kind = "reg", color = "#ce1414", title = "Joint Plot", fig_width = 18, fig_height = 20):
        sns.set(font_scale=1)
        plt.figure(figsize=(fig_width,fig_height))
        sns.jointplot(self.df.loc[:, var1], self.df.loc[:,var2], kind=kind, color=color)
        plt.title(title)
        return plt
...
class Evaluation:
    ...
    def confusion_matrix(self, y_predicted, target_names= None, normalize = True, fig_width = 20, fig_height = 20, title = "Confusion Matrix"):
        '''
        ...
        if normalize:
            ...
            sns.heatmap(con, annot=True,fmt='.2', cmap='Blues',xticklabels= y_unique , yticklabels= y_unique)
        else: 
            #con = np.zeros((classes,classes))
            #for x in range(classes):
             #   for y in range(classes):
             #       con[x,y] = cm[x,y]
            sns.heatmap(cm, annot=True,cmap='Blues',xticklabels= y_unique , yticklabels= y_unique, fmt='g')
        plt.title(title)
        return plt
    ...
    def auc_roc_plot(self, X_val, classifiers : list, classifier_names : list, fig_width = 15, fig_height = 10, title = 'ROC Curve Analysis' ):
        '''
